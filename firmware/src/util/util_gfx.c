 ...
 *****************************************************************************/
#include "../system.h"
#include "mishmash7pt7b.h"
#include "quarthin7pt7b.h"
#include "veramono5pt7b.h"
#include "veramono8pt7b.h"
...
#define __FONT_LARGE 		mishmash7pt7b
//#define __FONT_LARGE_THIN	quarthin7pt7b
#define __FONT_MEDIUM		VeraMono8pt7b
#define __FONT_SMALL		VeraMono5pt7b
 ...
 */
void util_gfx_draw_bmp_file(char *filename, uint8_t x, uint16_t y) {
...
void util_gfx_draw_bmp(bitmap_t *p_bitmap, uint16_t x, uint16_t y,
		uint16_t color) {
	int16_t i, j;
	uint16_t offset = 0;
...
uint8_t __util_gfx_draw_raw_file_inner(char *filename, int16_t x, int16_t y, uint16_t w, uint16_t h, void (*p_frame_callback)(uint8_t frame, void *p_data), bool loop,
	...
	do {
		...
		if (result != FR_OK) {
			char text[32];
			sprintf(text, "Error while seeking SD. Error code: %d", result);
			mbp_ui_error(text);
		}

//		uint32_t start = util_millis();
		...
		for (uint16_t i = 0; i < frames; i++) {
			...
			while (bytecount > 0) {
				...
				if (result != FR_OK) {
					...
					//Break out of frames for loop
					i = frames;
					//break out of rows while loop
					break;
				...
				st7735_push_colors_fast(sdbuffer, count);
			...
			if ((util_button_state() > 0 && loop) || m_stop) {
				loop = false;
...
//		if (frames > 4) {
//			uint32_t delta = util_millis() - start;
//			char message[32];
//			sprintf(message, "F:%d D:%d", (int) frames, (int) delta);
//			mbp_ui_popup("FPS", message);
//		}
...
void util_gfx_inverted_schedule_handler(void *p_data, uint16_t length) {
	bool *p_inverted = (bool *) p_data;
	st7735_inverted_set(p_inverted[0]);
	util_tilt_inverted_set(p_inverted[0]);
	util_gfx_invalidate();
}
...
void util_gfx_init() {
	...
	util_gfx_fill_screen(COLOR_BLUE);
}

void util_gfx_print(char *text) {
	GFXfont font = __get_font();
